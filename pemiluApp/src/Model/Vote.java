...
public class Vote {
    ...
    public static Vote getUserVote(String nik_user) throws SQLException {
        Database.connect();
        PreparedStatement sql = Database.prepareStatement("SELECT * FROM votes WHERE `nik_user`=?");
        sql.setString(1, nik_user);
        ResultSet rs = Database.query(sql);
        rs.next();
        Vote vote = new Vote(
    ...
    public static ArrayList<Vote> getAllTPSVotes(int id_tps) throws SQLException {
        Database.connect();
        PreparedStatement sql = Database.prepareStatement("SELECT * FROM votes WHERE `id_tps`=?");
        sql.setInt(1, id_tps);
        ResultSet rs = Database.query(sql);
        ArrayList<Vote> allVotes = new ArrayList<>();
        while (rs.next()) {
    ...
    public static ArrayList<Vote> getAllCalonVotes(int id_calon) throws SQLException {
        Database.connect();
        PreparedStatement sql = Database.prepareStatement("SELECT * FROM votes WHERE `id_calon`=?");
