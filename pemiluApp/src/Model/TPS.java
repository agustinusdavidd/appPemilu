...
public class TPS {
    ...
    public static ArrayList<TPS> getAll() throws SQLException {
        Database.connect();
        ResultSet rs = Database.query("SELECT * FROM TPS");
    ...
    public static TPS getById(int id) throws SQLException {
        Database.connect();
        PreparedStatement sql = Database.prepareStatement("SELECT * FROM TPS WHERE `id`=?");
        sql.setInt(1, id);
    ...
    public static TPS getByLokasi(String lokasi) throws SQLException {
        Database.connect();
        PreparedStatement sql = Database.prepareStatement("SELECT * FROM TPS WHERE `lokasi`=?");
    ...
    public static ArrayList<TPS> getByPanitia(String nik_panitia) throws SQLException {
        Database.connect();
        PreparedStatement sql = Database.prepareStatement("SELECT * FROM TPS WHERE `nik_panitia`=?");
    ...
    public static int delete(int id) throws SQLException {
        Database.connect();
        PreparedStatement sql = Database.prepareStatement("DELETE FROM TPS " +
                "WHERE `id`=?");
        sql.setInt(1,id);
        int rs = Database.update(sql);
        Database.disconnect();
